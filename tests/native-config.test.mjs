...
test("the CloudKit container is identical in the entitlement and the source", async () => {
  const [entitlements, model] = await Promise.all([
  ...
  const declared = model.match(/cloudKitContainer:\s*"(iCloud\.[^"]+)"/)?.[1];
  assert.equal(
    declared,
    entitled[0],
    "the mirror connection and the entitlement must name the same container",
  );
});
